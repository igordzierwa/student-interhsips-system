<template>
  <el-row>
    <el-col :span="24">
      <h4>Lista wysłanych wiadomości</h4>
      <el-table
        :data="model.list"
        style="width: 100%"
        id="SentMsgList"
...
export default {
  ...
  data() {
    return {
      componentName: "SentMessagesList",
      model: {
        list: [],
        userId: 1,
        toDeleteId: -1,
  ...
  methods: {
    handleLoadMessages() {
      this.axios
        ...
        .then((resp) => {
          this.axios({
            method: "GET",
            url: this.api + "/messages-sent/" + resp.data.id,
          })
            .then((resp) => {
              console.log(resp);
    ...
    handleConfirmDelete() {
      ...
      this.axios({
        method: "DELETE",
        url: this.api + "/messages-sent/" + this.model.toDeleteId,
      })
        .then((resp) => {
          console.log(resp);
          this.message({
